...
class QuickBite extends React.Component {
  ...
  IncrementItem = () => {
    if (this.state.quantity >= this.state.max) {
      // Do nothing if max limit is reached
    } else {
      this.setState({
  ...
  DecreaseItem = () => {
    if (this.state.quantity <= this.state.min) {
      // Do nothing if min limit is reached
    } else {
    ...
    return (
      <div className={"p-3 border-bottom " + this.props.itemClass}>
        {this.state.quantity > 0 && (
          <span className="count-number float-right">
            <button 
              ...
              className="btn btn-outline-secondary btn-sm left dec">
              <Icofont icon="minus" />
            </button>
            <input 
            ...
            <button 
              ...
              className="btn btn-outline-secondary btn-sm right inc">
              <Icofont icon="plus" />
            </button>
          </span>
        )}
        <Media>
          ...
          <Media.Body>
            <h6 className="mb-1">
              {this.props.title} {this.props.showBadge && (
                <Badge variant={this.props.badgeVariant}>{this.props.badgeText}</Badge>
              )}
            </h6>
